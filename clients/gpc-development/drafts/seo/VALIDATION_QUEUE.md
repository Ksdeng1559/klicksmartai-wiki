...
| `CLIENT-SCORE-gpc-development-2026-08-28.md` | client-score | $0 (derived from existing audit + DuckDB keyword data) | pending Dennis review | **New deliverable type** (registered in `_config/deliverables.md`). 23/100 CONDITIONAL tier, 4-dim weighted scoring (tech 30 / content 18 / local 12 / SERP 28). ROI: $148K Yr-1 / $284K Yr-2 run-rate on $8K spend (18.5× / 35.5×). Companion to existing audit + quote. Requires new `deliverables/seo/` subfolder + render via `seo-audit-report` skill once approved. |
