...
| Codex | financial model coding, SQL generation, Python analysis, validation tests, automation scripts |
...
Codex / Claude Code
    -> Python automation, schemas, tests, models, SQL, pipelines, and design-output implementation support

Validation Agent
    -> assumptions, citations, risk, contradiction checks, automation-output checks, and design-claim checks
...
## Agent Development Addendum: Test-Driven Output and Learning Loop

This addendum governs how individual agents are developed. It does not replace the strategic goals of the SpectraHoldings wiki.
...
No agent is production-ready until it repeatedly produces outputs that pass its defined tests and support a real business decision.

## Agent Development Addendum: Python Automation Layer

The automation layer for Spectra agents will be built primarily as Python scripts created, reviewed, and maintained through Codex and/or Claude Code.
...
Scripts must not silently fail.

If required data is missing, stale, malformed, or contradictory, the script must return a clear status such as:
...
Python scripts are the automation layer that moves data between sources, storage, validation, and outputs.
...
```text
Phase 1: Local DuckDB + Python scripts
Phase 2: Shared MotherDuck + version-controlled Python automation
...
- use Python automation outputs only when they pass validation
...
1. `DUCKDB-SCHEMA.md` — define storage tables for reports, assumptions, MCF draws, incentives, capital stack items, leadership clarifications, design assets, multimodal extraction records, agent tests, automation runs, and learning loop records.
2. `CLAUDE.md` — define long-context ingestion and synthesis workflow.
3. `CODEX.md` — define coding, SQL, financial model, Python automation, and design-output implementation standards.
4. `HERMES.md` — define recurring research and county monitoring workflows.
5. `automation/README.md` — define Python script standards, folder structure, dependencies, testing, and logging.
6. `agents/validation-agent.md` — define investor-readiness, assumption validation, automation-output validation, and design-claim validation rules.
7. `agents/agent-testing-governance.md` — define test-driven output standards for every Spectra agent.
8. `workflows/capital-stack-workflow.md` — define the complete funding stack process.
9. `workflows/mcf-deployment-workflow.md` — define facility draw, deployment, recycle, and reporting logic.
10. `workflows/agent-learning-loop-workflow.md` — define how agent outputs are reviewed, scored, corrected, and improved.
11. `workflows/python-automation-workflow.md` — define how Codex / Claude Code convert agent specs into Python scripts and tests.
12. `workflows/gemini-design-ingestion-workflow.md` — define Gemini visual extraction into design.md, OpenDesign, and HyperFrames inputs.
13. `workflows/multimodal-design-extraction-workflow.md` — define use of multimodal LLMs such as MiniMax M2.7 for visual extraction, cross-checking, and design-ready output creation.
14. `tests/agent-output-tests/` — store reusable pass / fail tests for each agent.
15. `tests/learning-loop-records/` — store feedback, corrections, and agent improvement history.
16. `automation/tests/` — store Python unit tests and fixtures.
17. `design/design-md/` — store generated `design.md` files.
18. `design/hyperframes/` — store HyperFrames scene plans and output instructions.
19. `design/opendesign/` — store OpenDesign-ready design instructions.

## Success Condition

The SpectraHoldings wiki succeeds when every project, county, capital source, leadership clarification, investor output, agent output, automation output, and design output can be:
...
Every automation script must connect defined inputs to validated, structured outputs.
